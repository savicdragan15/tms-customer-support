...
<div class="relative">
	<div class="vx-navbar-wrapper">
		<vs-navbar class="vx-navbar navbar-custom" :color="navbarColor" :class="classObj">
            ...
            <img v-bind:src="$auth.user().avatar" :alt="$auth.user().name" width="40" height="40" class="rounded-full shadow-md cursor-pointer block">
          </div>
					<vs-dropdown-menu class="vx-navbar-dropdown">
						<ul style="min-width: 9rem">
							...
							<li class="flex py-2 px-4 cursor-pointer hover:bg-primary hover:text-white" @click="$auth.logout()"><feather-icon icon="LogOutIcon" svgClasses="w-4 h-4"></feather-icon> <span class="ml-2">Logout</span></li>
						</ul>
					</vs-dropdown-menu>
				</vs-dropdown>
			</div>
